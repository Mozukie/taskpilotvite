...
router.post("/register", async (req, res) => {
  try {
    ...
    }else if (full_name.length < 3 || first_name.length > 50) {
      return res.status(400).json({ error: "Full name must be between 3 and 50 characters long!" });
    }else if (full_name.length < 3 || last_name.length > 50) {
      return res.status(400).json({ error: "Full name must be between 3 and 50 characters long!" });
    ...
    window.location.href = "/UserProfile"; // Redirect to login page after successful registration
